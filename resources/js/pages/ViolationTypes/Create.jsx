...
export default function CreateViolationType({ auth }) {
  ...
  const [tanggalAkhirBerlaku, setTanggalAkhirBerlaku] = useState(null);
  ...
  const handleSubmit = (e) => {
    ...
    post(route(`${resource}.store`), {
      onSuccess: () => {
        reset();
        // Tambahkan notifikasi sukses jika perlu, misal dengan SweetAlert atau toast
      },
    });
  };
  ...
  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Tambah Jenis Pelanggaran</h2>}
    >
      <Head title="Tambah Jenis Pelanggaran" />

      <Container>
        <Card title="Form Tambah Jenis Pelanggaran">
